  ...
  return (
    <section className="filter">
        {/* 
            ...
            - This filter panel is implemented as a fixed-position sidebar (drawer).
            - `fixed` removes it from the normal document flow, so it does NOT push
                the main content (products) down or sideways.
            - `left-0` and `top-0` pin the filter to the left side of the screen,
                starting from the top.
            - `h-screen` makes the filter span the full height of the viewport.
            - `w-72` gives the sidebar a fixed width.
            - Because it is fixed, the products section remains in its original position
                and the filter appears beside it, like a floating sidebar.
            - `z-40` ensures the filter stays above other page content.
        */}
        <div id="drawer-disable-body-scrolling" className={`fixed z-40 h-screen p-5 overflow-y-auto bg-white w-72 dark:bg-gray-800 transition-transhtmlForm left-0 top-0 transhtmlForm-none`} tabIndex="-1" aria-labelledby="drawer-disable-body-scrolling-label" aria-modal="true" role="dialog">
            ...
            <div className="py-4 overflow-y-auto">
                <ul className="text-slate-700 dark:text-slate-100">
                    {/* Radio buttons are used for price and rating to allow only one selection,
                        while checkboxes are used for other filters to allow multiple selections */}
                    <li className="mt-1 mb-5">
                    <p className="font-semibold my-1">Sort by</p>                      
                    <div className="flex items-center my-1">
                              {/* 
                                    // `state` holds current filter values: sort, ratings, bestSellerOnly, onlyInStock
                                    // `dispatch` updates the state when user interacts with checkboxes/radios
                                    // Using `dispatch` updates the values in the global `state` in the provider, 
                                    // which triggers a re-render and updates the filtered products automatically.
                                    // The `onChange` event is used to handle user interactions with checkboxes and radio buttons.
                                    // `checked` determines whether the checkbox or radio button is currently selected or not.
                                */}
